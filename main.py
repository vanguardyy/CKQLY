...
def send_message(to_user, access_token, region_name,weather, temp, tempn, wind_dir, note_ch, note_en):
    ...
    for key, value in birthdays.items():
        ...
        if birth_day == 0:
            birthday_data = "今天回国啦！".format(value["name"], value["name"])
        else:
            birthday_data = "距离修阔爱回国还有{}天".format(birth_day)
        # 将生日数据插入data
...
if __name__ == "__main__":
    ...
    weather, temp, tempn, wind_dir = get_weather(region)
    note_ch = config["note_ch"]
    note_en = config["note_en"]
    if note_ch == "" and note_en == "":
        # 获取词霸每日金句
        note_ch, note_en = "希望我的宝宝工作顺利天天快乐！<(￣︶￣)↗[GO!]","我对你的爱永远不会消失！"
    # 公众号推送消息
    for user in users:
